function initTemplates () {
  ...
  // init size vars
  let width, height, cartItemIndex;
  ...
  urlQueryString.map( value => {
    ...
    // set cart item index
      ...
      // set width
      cartItemIndex = value.slice(equalIndex, value.length);
  ...
  getDesignTemplates(width, height, cartItemIndex);

}

// call service to get design templates
function getDesignTemplates (width, height, cartItemIndex) {
  ...
  callTemplatesService(qData)
    .then( data => {
      displayDesignTemplates(data, cartItemIndex);
    });
}

// display design templates
function displayDesignTemplates (templates, cartItemIndex) {
  ...
  templates.map( template => {
    ...
    const templateHtml = `
      <div class="product-box">
        <a href="/createDesign/?templateId=${template[1]}&cartItemIndex=${cartItemIndex}">
